# functions
  ...
  getInfectivityProfile2 <- function(times, params, CDF = FALSE) {
    if (CDF) return(pgammaShift(x = times, shape = params$shape, rate = params$rate, shift = params$shift))
    else return(dgammaShift(x = times, shape = params$shape, rate = params$rate, shift = params$shift))
  }
