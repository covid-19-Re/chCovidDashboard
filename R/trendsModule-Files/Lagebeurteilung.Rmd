...
r_cases_timeHosp <- r_cases_timeHosp[r_cases_timeHosp$date == max(r_cases_timeHosp$date), ]
...
r_hosp_timeDeath <- r_hosp_timeDeath[r_hosp_timeDeath$date == max(r_hosp_timeDeath$date),]
r_cases_timeDeath <- r_cases_timeDeath[r_cases_timeDeath$date == max(r_cases_timeDeath$date),]
```
...
Über die ganze Schweiz zeigt die SARS-CoV-2-Epidemie einen rückgängigen Verlauf. Die effektive Reproduktionszahl *R~e~* beträgt[^1]:
...
<!--- TODO add r_cases_timeHosp, r_cases_timeDeath --->
...
* `r format(round(r_deaths$median_R_mean, 2), decimal.mark = ",")` (95% UI: `r format(round(r_deaths$median_R_lowHPD, 2), decimal.mark = ",")`-`r format(round(r_deaths$median_R_highHPD, 2), decimal.mark = ",")`) aufgrund der Todesfälle und `r format(round(r_hosp_timeDeath$median_R_mean, 2), decimal.mark = ",")` (95% UI: `r format(round(r_hosp_timeDeath$median_R_lowHPD, 2), decimal.mark = ",")`-`r format(round(r_hosp_timeDeath$median_R_highHPD, 2), decimal.mark = ",")`) aufgrund der Hospitalisationen (per `r day(r_deaths$date)`.`r month(r_deaths$date)`.`r year(r_deaths$date)`)

Die kumulierte Anzahl der bestätigten Fälle über die letzten 14 Tage liegt bei `r round(cases14d_CH$valueNorm14daySum, 0)` pro 100'000 Einwohner. Für die Fallzahlen sind Vergleiche zu den Vorwochen momentan schwierig da über die Feiertage eher weniger getestet wurde. Dies drückte sich um Weihnachten in einer erhöhten Positivität aus.

Aufgrund von Meldeverzögerungen und Fluktuationen in den Daten müssen die Schätzwerte jedoch mit Vorsicht interpretiert werden und könnten möglicherweise nachkorrigiert werden.
Eine Betrachtung der sieben Grossregionen der Schweiz zeigt, dass auch dort die Trends in den Hospitalisierungen rückläufig und Todesfällen konstant bis rückläufig sind.

<!--- , der hohen Testpositivität, einer variierenden Anzahl Tests und der Feiertage müssen die einzelnen Schätzwerte von *R~e~* mit Vorsicht interpretiert werden. ---> 
Die Analyse wird gestützt durch die beobachteten Verdopplungs- bzw. Halbwertszeiten der <!--- bestätigten Fälle und---> 
Hospitalisationen und Todesfälle über die letzten 14 Tage.[^2] 
Die Anzahl der COVID-19-Patienten auf Intensivstationen (`r range(icu$count)[1]`-`r range(icu$count)[2]`)[^3] <!---und der täglichen Todesfälle (`r range(deaths$count)[1]`-`r range(deaths$count)[2]`)[^4] --->
bleiben über die letzten 14 Tage auf hohem Niveau relativ stabil. 
...
Seit dem 1. Oktober 2020 weist das Bundesamt für Gesundheit `r format(deaths_total, big.mark = "'")` laborbestätigte Todesfälle aus.[^5] Die Kantone meldeten in dieser Zeit `r format(corona_data_deaths, big.mark = "'")` Todesfälle.[^6] Die Sterblichkeitsstatistik vom Bundesamt für Statistik weist über die letzten 11 gemeldeten Wochen eine deutliche Übersterblichkeit in der Altersgruppe 65 Jahre und älter auf.[^7] Diese Übersterblichkeit zeigt sich in allen Grossregionen der Schweiz mit Ausnahme der Région lémanique.[^8]
