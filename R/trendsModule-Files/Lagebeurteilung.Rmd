...
Der Impfschutz gegen schwere Erkrankung ist auch für die Delta-Variante hoch. Die Wirksamkeit liegt bei ungefähr 96% [91-98%] basierend auf Daten aus Grossbritannien[^47],[^44] und 88% (95% UI: 78,9-93,2%) basierend auf Daten aus Israel[^39],[^51].
...
Gemäss dem neuen Preprints aus Israel[^51] und den Analysen von Public Health England[^52] scheint der Schutz vor schweren Erkrankungen über die Zeit abzufallen.
...
Transmission:
Wie oben beschrieben, verringert die Impfung das Risiko einer Ansteckung bei einer geimpften Person um etwa 50%.
Die Impfung scheint auch die Transmission, die von einer infizierten geimpften Person ausgeht, zu reduzieren (aufgrund von kürzerer infektiöser Phase[^46],[^49], und milderer Symptomatik[^49]).
Das genaue Ausmass der Reduktion kann noch nicht genau beziffert werden.
Während ein Bericht des CDC[^38] darauf hindeutet, dass die Viruslast von Delta in Geimpften ähnlich hoch ist wie in Ungeimpften, deuten die Daten der REACT-Studie aus England[^45] darauf hin, dass Geimpfte niedrigere Viruslasten haben.
