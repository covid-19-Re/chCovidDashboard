...
In der Schweiz ist seit Kalenderwoche 51 2021 die Omikron Variante dominant.
Die Zahl der bestätigten Fälle ist in den darauffolgenden zwei Wochen schnell angestiegen und hat ein sehr hohes Niveau erreicht.
Der Anstieg ist nun deutlich abgeflacht: der R-Wert ist diese Woche nicht mehr über 1
Der flachere Anstieg der bestätigten Fälle könnte darauf hindeuten, dass ein Höchststand der Omikron-Welle erreicht sein könnte.
Diese Interpretation wird unterstützt durch Messungen von SARS-CoV-2 im Abwasser[^23], die auf keine grossen Diskrepanzen mit den gemeldeten Fällen hindeuten.
<!---- Allerdings könnte er auch durch die Grenzen der Erfassung durch Tests zustande kommen, denn die Fallzahlen und der Anteil der positiven Tests sind höher als je zuvor in der Pandemie. ---->


### Dynamik

<!------- UPDATE -------->
Seit Mitte Dezember 2021 ist der R-Wert signifikant über 1, hat Ende Dezember einen Höchststand von 1.6 erreicht und ist seitdem gesunken. Der aktuellste Wert ist nicht mehr signifikant über 1.
...
Insbesondere sind Spitalmeldungen zur Zeit unvollständig.
...
mit einer Rate von `r valueUIchange(rankingCHEcases)` pro Woche zu.
Die gemeldeten Hospitalisierungen fielen mit einer Rate von `r valueUIchange(rankingCHEhospitalizations)` pro Woche,
wobei diese Zahl wegen den oben erwähnten Meldeverzögerungen verfälscht ist.
...
Die Zahl der Fälle nahm in den Altersgruppen der 0-9, 10-19, 30-39 und 40-49 Jährigen signifikant zu.
Mit der Rate von 74% (95% UI: 54-96%) war der Anstieg in den 0-9 Jährigen am stärksten.
Der Anstieg in den 0-9 Jährigen war mit 70% (95% UI: 55-86%) pro Woche am schnellsten.
...
Mittlerweile wurde Omikron in 89 Ländern nachgewiesen (Stand 16. Dezember 2021) [^302],[^303],[^324], anfänglich meist verknüpft mit Einreise von Ländern aus dem Süden Afrikas.
In der Schweiz wurde die Variante etwa 6000-mal nachgewiesen und hatte in der zweiten Kalenderwoche 2022 einen Anteil von 93% unter allen sequenzierten Proben[^25].
<!---- und 8% unter den sequenzierten Proben, die auf GISAID als Teil des Surveillance Programms deklariert sind ---->
...
Für die Schweiz sind die geschätzten Werte der Wachstumsrate mit 0,17 (95% UI: 0,16-0,18) pro Tag niedriger als die Schätzungen aus Südafrika und Grossbritanien.
Der relative Wachstumvorteil gegenüber Delta in der Schweiz ist mit 126% (95% UI: 120-132%) geringer als in Südafrika und Grossbritannien, was möglicherweise auf eine vermehrte Verimpfung von Moderna und keinerlei Verimpfung von Oxford/AstraZeneca in der Schweiz[^5] zurückzuführen ist.
...
Gemäss einem neuem Preprint sind T-Zell-Antworten in Geimpften und Genesenen gegen Omikron Stachelprotein nur wenig schächer als gegen die ursprüngliche Variante[^341]. 
...
Die UKHSA[^337] schätzt, dass das Hospitalisierungsrisiko einer Person, die symptomatisch mit Omikron infiziert ist, verglichen mit einer Delta-infizierten Person um 68% (95% UI: 42-82%) reduziert ist. Gemäss diesem Bericht ist das Hospitalisierungsrisiko nach einer Omikron-Infektion bei Personen, die zwei Impfdosen erhalten haben, 65% niedriger und bei Personen, die drei Impfdosen erhalten haben, 81% niedriger als bei Ungeimpften


#### Schwere der Infektion

Laut einer Studie des CDC[^338] an 6 Personen, die teilweise geimpft oder genesen waren, ist die Inkubationszeit einer Infektion mit Omikron mit ungefähr 3 Tagen kürzer als die der früher zirkulierenden Varianten.
Laut einem Bericht der Harvard Universität[^343] ist die mittlere Dauer einer Omikron-Infektion mit 9,9 Tagen (95% UI: 8.8-10.9 Tagen) etwa einen Tag kürzer als eine Delta-Infektion und die Peak-Viruslast niedriger.
Die Analyse in dieser Studie war nicht bezüglich des Immunstatus der Probanden bereinigt.
...
Wenn man nicht fuer den Impfstatus korrigiert, kommt man zum Schluss, dass Infektionen mit Omikron seltener zu einer Hospitalisation führen als Infektionen mit Delta.
In einem Preprint aus Südafrika[^329] ist die Odds Ratio (bereinigt bezüglich demographischer und klinischer Kofaktoren und dokumentierter Genesung, aber nicht Impfstatus) der Hospitalisation nach einer wahrscheinlichen Omikron-Infektion (basierend auf «S gene target failure» Daten; verglichen mit Delta-Infektionen) 0.2 (95% UI: 0.1-0.3).
Eine neue Studie aus Südafrika[^342] schätzt eine 4-mal niedrigere Wahrscheinlichkeit einer IPS-Einweisung und des Todes nach einem Spitaleintritt.
Eine neue, noch nicht begutachtete Studie mit einer grossen Stichprobe aus den USA[^339] schätzt, dass Omikron-Infektionen bezüglich vieler angewendeten Masse höchstens halb so schwer verlaufen wie Delta-Infektionen. Insbesondere ist die Hospitalisierungsrate in der Omikron-Kohorte 1,8%, während sie in der Delta-Kohorte 4,0% beträgt,
und die Rate der IPS-Einweisungen 0.3% nach einer Infektion mit Omikron statt 0.8% mit Delta.
<!--- ED visit: 4.55% vs. 
15.22% (risk ratio or RR: 0.30, 95% CI: 0.28-0.33); hospitalization: 1.75% vs. 3.95% (RR: 0.44, 
95% CI: 0.38-0.52]); ICU admission: 0.26% vs. 0.78% (RR: 0.33, 95% CI:0.23-0.48); 
mechanical ventilation: 0.07% vs. 0.43% (RR: 0.16, 95% CI: 0.08-0.32). In children under 5 
years old, the overall risks of ED visits and hospitalization in the Emergent Omicron cohort were 
3.89% and 0.96% respectively, significantly lower than 21.01% and 2.65% in the matched Delta 
cohort (RR for ED visit: 0.19, 95% CI: 0.14-0.25; RR for hospitalization: 0.36, 95% CI: 0.19-
0.68). ---->
[^342].
...
Einer Studie des UKHSA zufolge [^337] ist die Hazard Ratio eines Spitaleintritts nach einer Omikron-Infektion 0.33 (95% UI: 0.30 to 0.37), was einer 67% niedrigeren Hospitalizationsrate als nach einer Delta-Infektion entspricht.
<!--- UPDATE! [^337]
...
---->
Gemäss einer Studie[^322] des Imperial College (beruhend auf ähnlichen Datenquellen wie die UKHSA Studie[^337]) ist die Hazard Ratio 0.8 (95% UI 0.75-0.85).
...
Es ist wichtig zu betonen, dass die niedrigere Hospitalisationsrate von Omikron auf der Populationsebene schnell durch die stark erhöhte Ausbreitung aufgewogen werden könnte. 
