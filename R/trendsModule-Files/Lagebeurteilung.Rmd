...
Bis Ende Juni 2021 war bei all diesen Indikatoren ein kontinuierlicher Rückgang der Epidemie zu verzeichnen. Seit Juli 2021 ist ein stetiger Anstieg zu verzeichnen.
<!------ UPDATE  -------->
...
<!------- UPDATE -------->
Basierend auf den aktuellen Daten schätzen wir, dass die SARS-CoV-2 Epidemie seit rund sechs Wochen kontinuierlich ansteigt. Seit Anfang Juli verlangsamt sich dieser Anstieg; der 7-Tageschnitt der schweizweiten Reproduktionszahl ist bei `r valueUIr(r_mean7d, uiText = "")`; dies reflektiert das Infektionsgeschehen vom `r r_mean7d_dateRange_str`[^1].
...
um `r valueUIchange(rankingCHEcases)` pro Woche und
die Hospitalisationen um `r valueUIchange(rankingCHEhospitalizations)`.
Todesfälle sanken um `r valueUIchange(rankingCHEdeaths)`.
<!---- UPDATE ---->
...
Wir beobachten einen Anstieg der Zahl der Fälle in allen Altersgruppen, der in allen Gruppen ausser bei den 65 bis 74-Jährigen signifikant ist.
...
Dennoch stiegen Hospitalisierungen in den Altergruppen der 35 bis 44-Jährigenund 55 bis 64-Jährigen von 23. Juli bis 5. August 2021 signifikant an.
<!------ UPDATE -------->
...
in der Kalenderwoche 30 eine Häufigkeit von 97%
unter den sequenzierten Fällen[^25].
Wegen Verzögerungen, mit denen die Sequenzen erfasst werden, können sich diese Häufigkeit, vor allem in Kalenderwoche 30, noch ändern.
Aus diesem Anstieg der Häufigkeit von Delta kann man einen Transmissionsvorteil gegenüber Alpha von 75-83% berechnen[^25].
<!--- UPDATE cov-spectrum.ethz.ch-> click on B.1.617.2 and look at the number/percentage by calendar week on the right graph  ---->

Die früher in der Schweiz beobachteten SARS-CoV-2-Varianten, darunter die von März bis Juni 2021 dominierende Alpha-Vaiante, aber auch die Beta- und Gamma-Varianten, die immer selten waren, haben eine Gesamthäufigkeit von <2% in den letzten Wochen[25].
...
Delta scheint schwerere Verläufe zu verursachen als die zuvor in der Schweiz dominierenden Stämme. Die Daten aus Schottland[^37] deuten darauf hin, dass eine Infektion mit Delta rund doppelt so häufig zur Hospitalisierung führt wie eine Infektion mit Alpha (hazard ratio 1,39-2,47). Daten aus Kanada zeigen einen Anstieg des Hospitalisierungsrisikos um 120% (93-153%) gegenüber den in 2020 zirkulierenden Stämmen[^36].

Der Impfschutz gegen symptomatische Infektionen mit Delta ist, gemäss eines Berichtes von Public Health England[^41], reduziert: 79% (78-80%) (Wirksamkeit gegen Alpha: 89% [87-90%]).
Studien welche die Wirksamkeit von mRNA Impfstoffen gegen eine Infektion (einschliesslich asymtopmatischer) quantifizieren sind am Laufen.
...
Die Daten aus Israel deuten zudem darauf hin, dass die Wirksamkeit von Impfungen gegen symptomatische Infektionen bei 40.5% [8.7-61,2%] [^39] liegen.
Daten aus England deuten auf 79% [78-80%] [^41] (siehe auch [^42], [^36]; Pfizer/BioNtech) und Daten aus Kanada auf 85% [78-89%] [^43] (Pfizer/BioNtech und Moderna).
...
Inwieweit die Impfung gegen Transmission schützt, ist noch nicht abschliessend geklärt.
...
jedem Fall darauf hin, dass Geimpfte, die mit Delta infiziert sind, das Virus übertragen zu können.

Der Impfschutz gegen schwere Erkrankung ist nach wie vor hoch. Die Wirksamkeit liegt bei ungefähr 96% [91-98%] basierend auf Daten aus Grossbritannien[^41],[^44] und 88% [78,9-93,2%] basierend auf Daten aus Israel [^39]. Das bedeutet, dass etwa 9 von 10 Hospitalisationen verhindert werden können durch Impfung.
