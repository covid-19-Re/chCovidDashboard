...
mostRecentVariant <- variants %>%
  filter(
    lab == "Total",
    year == max(year)) %>%
  filter(week == max(week))

startOfVariantWeek <- parse_date_time(str_c(mostRecentVariant$year_week,"-1"), "Y-W-w")

```

```{r printFuns, echo = FALSE}
valueUI <- function(value, lower, upper, uiText = "UI", uiSep = "-", unit = "") {
  glue(
    "{format(round(value, 2), decimal.mark = ',')}{unit} ",
    "({uiText}: {format(round(lower, 2), decimal.mark = ',')}{unit}",
    "{uiSep}",
    "{format(round(upper, 2), decimal.mark = ',')}{unit})"
  )
}

valueUIr <- function(df, uiText = "95% UI", uiSep = " bis ", unit = "") {
  valueUI(df$median_R_mean, df$median_R_lowHPD, df$median_R_highHPD, uiText = uiText, uiSep = uiSep, unit = unit)
}
valueUIchange <- function(df, uiText = "UI", uiSep = " - ", unit = "%") {
  valueUI(df$estimate * 100, df$lower * 100, df$upper * 100, uiText = uiText, uiSep = uiSep, unit = unit)
}
```
...
aufgrund der Verzoegerung von Infektion und Eintreten eines Ereignis (Fallbestätigung, Hospitalisationen, Todesfall).
...
und die Todesfaelle um `r valueUIchange(rankingCHEdeaths)`.
...
Die Positivität liegt bei `r format(round(mostRecentTestPositivity$testPositivity, 2), decimal.mark = ',')`
...
(die Änderung war `r valueUIchange(rankingCHEicu)`).

Die Zahl der täglichen täglichen Todesfälle über die letzten 14 Tage war zwischen `r range(deaths$count)[1]`-`r range(deaths$count)[2]`[^4].
...
Die Sterblichkeitsstatistik vom Bundesamt für Statistik weist über die letzten 11 gemeldeten Wochen <!--- CHECK --->
...
In Kalenderwoche `r mostRecentVariant$week`, `r mostRecentVariant$year` (ab dem `r format(startOfVariantWeek, "%d.%m.%Y")`)
wurde die Häufigkeit von B.1.1.7 in der Schweiz auf etwa `r format(round(mostRecentVariant$p, 2), decimal.mark = ',')`% geschätzt
(95%-Unsicherheitsinterval `r format(round(mostRecentVariant$p_lower, 2), decimal.mark = ',')`%-`r format(round(mostRecentVariant$p_upper, 2), decimal.mark = ',')`%;
das reflektiert das Infektionsgeschehen etwa 10 Tage früher[^14]) <!---- TODO Zahlen in diesem Satz automatisch holen von https://ibz-shiny.ethz.ch/covidDashboard/variant-plot/index.html (most recent total estimate)! ---->
