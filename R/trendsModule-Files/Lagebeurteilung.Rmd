...
<!------- UPDATE -------->
Basierend auf den aktuellen Daten schätzen wir, dass die SARS-CoV-2 Epidemie seit einigen Wochen kontinuierlich ansteigt. Der 7-Tageschnitt der schweizweiten Reproduktionszahl ist bei `r valueUIr(r_mean7d, uiText = "")`; dies reflektiert das Infektionsgeschehen vom `r r_mean7d_dateRange_str`[^1].
...
Todesfälle traten nur vereinzelt auf und lassen eine Berechnung des zeitlichen Trends auf der Basis der letzten 14 Tage nicht zu (aber siehe *R~e~* auf Basis der Todesfälle oben).
...
Wir beobachten steigende Fallzahlen in allen Altersgruppen. Hospitalisierungszahlen sind in allen Altersgruppen im einstelligen Bereich, was die Schätzung eines zeitlichen Trends erschwert.
<!------ UPDATE -------->
<!------ TOO DETAILLED:
...
In den letzten 14 Tagen gab es höchstens einen laborbestätigten Todesfall pro Tag.
<!---- UPDATE Die Zahl der täglichen laborbestätigten Todesfälle über die letzten 14 Tage war zwischen `r range(deaths$count)[1]` und `r range(deaths$count)[2]`[^4]. ---->
<!--- 
...
in der Kalenderwoche 29 eine Häufigkeit von 98%
unter den sequenzierten Fällen [^25].
Wegen Verzögerungen, mit denen die Sequenzen erfasst werden, können sich diese Häufigkeit, vor allem in Kalenderwoche 29, noch ändern.
...
Im März 2021 war Alpha (B.1.1.7) die dominante Virusvariante in der Schweiz geworden[^15],[^16], deren Übertragungsrate um etwa 50% höher als die des Wildtyps ist [^13],[^18],[^16]. 
Die Beta (B.1.351) und Gamma Varianten (P.1), die seit dem ersten Quartal 2021 in der Schweiz nachgewiesen wurden, sind derzeit sehr selten, mit einer durchschnittlichen Häufigkeit von 0% bzw. 0,3% in den letzten 14 Tagen [^25].
<!--- UPDATE cov-spectrum.ethz.ch-> see percentages on the left, which is the average over the past 14 days ---->

Delta dominiert mittlerweile die Epidemie in vielen Ländern, darunter Grossbritannien[^32], den Vereinigten Staaten von Amerika, Frankreich, Deutschland, Italien und Portugal[^25].
Der Übertragungsvorteil von Delta gegenüber Alpha wurde auf 56% (95% Konfidenzintervall: 34%-81%) geschätzt [^33].
Einem Preprint aus Kanada[^37] und einer Studie aus Schottland[^38] zufolge ist die Hospitalisierungsrate von Delta im Vergleich zu Alpha um 74%-85% erhöht.
Eines Preprints von Public Health England[^26] zufolge ist der Schutz der Impfung gegen Delta reduziert:
...
Zeitverzögert steigen dort nun auch die Zahl der Spitaleintritte wieder an[^34].
...
Wir rechnen auch mit einem weiteren Anstieg von Hospitalisierungen, der wegen der geringeren Impfdeckung in der Schweiz verglichen mit Grossbritannien[^35] voraussichtlich einen grösseren Anteil der Infizierten betreffen wird.
...
[^34]: https://coronavirus.data.gov.uk/details/healthcare
[^35]: https://sciencetaskforce.ch/en/scientific-update-of-6-july-2021/
[^36]: https://depositonce.tu-berlin.de/handle/11303/13423
[^37]: https://doi.org/10.1101/2021.07.05.21260050
[^38]: https://www.thelancet.com/journals/lancet/article/PIIS0140-6736(21)01358-1/fulltext
