...
reRoundDigits <- 2
percentRoundDigits <- 0

valueUIr <- function(df, uiText = "95% UI", uiSep = "-", unit = "", digits = reRoundDigits) {
  valueUI(df$median_R_mean, df$median_R_lowHPD, df$median_R_highHPD,
    uiText = uiText, uiSep = uiSep, unit = unit, digits = digits)
}
valueUIchange <- function(df, uiText = "UI", uiSep = " bis ", unit = "%", digits = percentRoundDigits) {
...
wurde die Häufigkeit von B.1.1.7 in der Schweiz auf etwa `r format(round(mostRecentb117$p, percentRoundDigits), decimal.mark = ',')`% geschätzt
(95%-Unsicherheitsinterval `r format(round(mostRecentb117$p_lower, percentRoundDigits), decimal.mark = ',')`% bis
`r format(round(mostRecentb117$p_upper, percentRoundDigits), decimal.mark = ',')`%;
