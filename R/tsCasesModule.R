      ...
      fluidRow(
        column(3,
          bootstrapPanel(heading = "Parameter", class = "panel-primary",
            radioButtons(inputId = ns("event"),
              label = "Clinical event",
              choices = events,
              selected = events[1], inline = TRUE),
            checkboxInput(inputId = ns("display_prob"),
              label = tags$b("Clinical event probability given:")),
            disabled(radioButtons(inputId = ns("given"),
              label = NULL,
              choices = events,
              selected = events[1], inline = TRUE))
          ),
          bootstrapPanel(class = "panel-primary",
            ...
            pickerInput(inputId = ns("age_groups"),
              label = "Age groups",
              choices = ageGroups,
              selected = ageGroups,
              multiple = TRUE,
              options = list(size = 8)),
            actionGroupButtons(inputIds = c(ns("all_ages"), ns("clear_ages")),
              labels = c("Select all", "Clear"),
              size = "xs"),
            checkboxInput(inputId = ns("compare_ages"), label = "Compare ages"),
            pickerInput(inputId = ns("cantons"),
              label = "Cantons",
              choices = cantons,
              selected = cantons,
              multiple = TRUE,
              options = list(size = 8)),
            actionGroupButtons(inputIds = c(ns("all_cantons"), ns("clear_cantons")),
              labels = c("Select all", "Clear"),
              size = "xs"),
            checkboxInput(inputId = ns("compare_cantons"),
              label = "Compare cantons"),
            radioButtons(inputId = ns("travel"),
              label = "Import status",
              choices = travelChoices,
              selected = travelChoices[1],
              inline = TRUE),
            checkboxInput(inputId = ns("compare_travel"),
